...
const SelectHeader = ({ workspaceId, isTodoList = false, isFull = false, isHidden = false }: SelectHeaderProps) => {
  ...
  return (
    <header
      className={`${isFull ? 'w-full' : ' '} ${isHidden ? 'hidden' : 'flex flex-row'} ${isTodoList ? 'w-full lg:w-[384px]' : ''} sticky top-0 items-center justify-between z-10 bg-white pt-[14px] px-[16px] pb-[12px] lg:bg-[#F4F4F6] lg:h-[84px] lg:flex`}
    >
